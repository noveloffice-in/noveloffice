...
def test():
	...
	report_filters = {'company': 'VIBGYOR NET CONNECTIONS PVT LTD', 'from_date': from_date, 'to_date': to_date, 'warehouse': 'Kudlu Gate - VNCPL'}

	report_data = execute(report_filters)
	# print(report_data)
	...
	if existing_doc:
		so_doc = frappe.get_doc('Item-Wise Sales Register Custom Report', f'{from_date}-{to_date}')
	else:
			...
			for index, values in enumerate(item):
				...
				if index != length - 1:
					...
					if sales_order:
						...
						customer_po = None
					prev = values['sales_order']
					report_values.append({'item_code': values['item_code'],
										...
										'receivable_account': values['debit_to'],
										...
										'output_tax_cgst__90_rate': values['output_tax_cgst_@_9.0_rate'],
										'output_tax_cgst__90_amount': values['output_tax_cgst_@_9.0_amount'],
										'output_tax_igst__180_rate': values['output_tax_igst_@_18.0_rate'],
										'output_tax_igst__180_amount': values['output_tax_igst_@_18.0_amount'],
										'output_tax_sgst__90_rate': values['output_tax_sgst_@_9.0_rate'],
										'output_tax_sgst__90_amount': values['output_tax_sgst_@_9.0_amount'],
	...
	writer = pd.ExcelWriter(f'/home/frappe/frappe-bench/apps/erpnext/erpnext/selling/report/automated_reports_for_logistics/excel_sheets/Itemwise Sales & As per Sales-{current_date}.xlsx', engine='xlsxwriter')
	...
	if existing_doc:
		so_doc = frappe.get_doc('As per Sales Monthly Summary', f'{from_date}-{to_date}')
	else:
	...
	new_doc.report_name = f'Itemwise Sales & As per Sales-{current_date}'
	new_doc.insert()
	url = [{'file_url' : ''}]
	
	file_data = open(f'/home/frappe/frappe-bench/apps/erpnext/erpnext/selling/report/automated_reports_for_logistics/excel_sheets/Itemwise Sales & As per Sales-{current_date}.xlsx', 'rb').read()
	file_attach = save_file(f'Itemwise Sales & As per Sales-{current_date}.xlsx', file_data, 'Logistics Report Attachments', new_doc.name)
	...
	subject = f'Itemwise Sales & As per Sales report - {current_date}'
	message = '''Hello Team,<br>
	<br>
	PFA, for the updated Itemwise Sales & As per Sales report as of 7:00 AM today.'''
	frappe.sendmail(
