...
def create_gross_margin():
	# def calculate_fiscal_year():
	#     current_date = frappe.utils.getdate()
	#     fiscal_year_start = frappe.utils.getdate(f"{current_date.year}-04-01")
	#     if current_date < fiscal_year_start:
	#         fiscal_year = current_date.year - 1
	#         fiscal_stop_year = current_date.year
	#     else:
	#         fiscal_year = current_date.year
	#         fiscal_stop_year = current_date.year + 1
	
	#     return [fiscal_year, fiscal_stop_year]
		
	# result = calculate_fiscal_year()
	# fiscal_start_year = result[0]
	# fiscal_stop_year = result[1]
	...
	report2 = frappe.get_doc('Report', 'Purchase Order Analysis')
	report_filters2 = {'company': 'VIBGYOR NET CONNECTIONS PVT LTD', 'from_date': from_date, 'to_date': to_date}
	columns2, report_data2 = report2.get_data(filters=report_filters2)
	
	# log(report_data)
	existing_doc = frappe.get_list('Gross Margin Report', filters = {'from_date': str(from_date),
																					'to_date': str(to_date)
	})
	if existing_doc:
		so_doc = frappe.get_doc('Gross Margin Report', f'{str(from_date)}-{str(to_date)}')
	else:
		so_doc = frappe.new_doc('Gross Margin Report')
		so_doc.from_date = str(from_date)
		so_doc.to_date = str(to_date)

	
	set1 = {item['item_code']: item['amount']/item['qty'] for item in reversed(report_data2) if type(item) != list}
	# print(set1)
	report_values = []
	for item in report_data:
		region = frappe.db.sql(f'select region from `tabCustomer` where name = "{item["customer_name"]}";')
		if region:
			region = region[0][0]
		else:
			region = None
		po = frappe.db.sql(f'select po_no from `tabSales Order` where name = "{item["sales_order"]}";')
		if item['item_code'] in set1:
			buy_price = set1[item['item_code']]
		else:
			buy_price = 0
		if po:
			po= po[0][0]
			...
			po = None
		if item['rate'] > 0:
			margin = (item['rate'] - buy_price)/item['rate']
		else:
			margin = None
		if_cust = frappe.get_list('Customer', filters = {'customer_name': item['customer_name']})
		if if_cust:
			cust_doc = frappe.get_doc('Customer', if_cust[0])
			customer = cust_doc.name
		else:
			if_sup = frappe.get_list('Supplier', filters = {'supplier_name': item['customer_name']})
			if if_sup:
				sup_doc = frappe.get_doc('Supplier', if_sup[0])
				customer = sup_doc.name
		report_values.append({'sales_order': item['sales_order'],
							  'invoice': item['invoice'],
							  'invoice_date': item['posting_date'],
							  'region': region,
							  ...
							  'buy_price': buy_price,
							  'purchase_amount': buy_price * item['stock_qty'],
							  'margin': margin
							  })
	print(report_values)
	...
	so_doc.set('report', {})
	so_doc.set('report', report_values)
	so_doc.save()


	writer = pd.ExcelWriter(f'/home/frappe/frappe-bench/apps/erpnext/erpnext/selling/report/automated_reports_for_logistics/excel_sheets/Gross Margin Report-{current_date}.xlsx', engine='xlsxwriter')
	df.to_excel(writer, sheet_name='Gross Margin', startrow=1, header=False, index=False)
	workbook = writer.book
	worksheet = writer.sheets['Gross Margin']
	...
	worksheet.set_column(max_col - 1, max_col - 1, 12, percentage_format)
	writer.close()

	new_doc = frappe.new_doc('Logistics Report Attachments')
	new_doc.date = current_date
	new_doc.report_name = f'Gross Margin Report-{current_date}'
	new_doc.insert()
	url = [{'file_url' : ''}]
	
	file_data = open(f'/home/frappe/frappe-bench/apps/erpnext/erpnext/selling/report/automated_reports_for_logistics/excel_sheets/Gross Margin Report-{current_date}.xlsx', 'rb').read()
	file_attach = save_file(f'Gross Margin Report-{current_date}.xlsx', file_data, 'Logistics Report Attachments', new_doc.name)
	...
	new_doc.save()
	print(url)
	recipient = ['ashish.k@vibgyornet.com']
	# recipient = ['mohammed.s@noveloffice.in']
	subject = f'Gross Margin report - {current_date}'
	message = '''Hello Team,<br>
	<br>
	PFA, for the updated Gross Margin report as of 7:00 AM today.'''
	frappe.sendmail(
