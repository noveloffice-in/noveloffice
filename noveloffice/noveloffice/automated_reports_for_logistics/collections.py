...
def validate_filters(filters, account_details):
	...
	if filters.get("account"):
		frappe.msgprint(f'{filters.get("account")}')
...
last_day = frappe.utils.get_last_day(current_date).day
filters = {'company': 'VIBGYOR NET CONNECTIONS PVT LTD',
			'from_date': f'{current_year}-{current_month}-01', 
			'to_date': f'{current_year}-{current_month}-{last_day}', 
			'account': ['VNCPLHDFC08852320000048 - VNCPL'], 
			'group_by': 'Group by Voucher (Consolidated)',
			'account_currency': 'INR',
			'include_dimensions': 1,
			'include_default_book_entries': 1}
def create_collection_report():
	doc = frappe.db.sql(f'''select distinct customer, region from `tabSales Order`;''', as_dict = True)

	cust_list = []

	for item in doc:
		cust_list.append(item['customer'])
	...
	existing_doc = frappe.get_list('Collections Report - GL', filters = {'from_date': str(from_date),
																	  'to_date': str(to_date)})

	if existing_doc:
		so_doc = frappe.get_doc('Collections Report - GL', f'{str(from_date)}-{str(to_date)}')
	else:
		so_doc = frappe.new_doc('Collections Report - GL')
	...
	for item in report_data:
		if skip:
			skip = False
			continue
		# if item.against in cust_list:
		for values in item:
			if values.against in cust_list:
				print(values)
				for info in doc:
					if info['customer'] == values.against:
						region = info['region']

				report_values.append({'against_account': values.against,
	...
	writer = pd.ExcelWriter(f'/home/frappe/frappe-bench/apps/erpnext/erpnext/selling/report/automated_reports_for_logistics/excel_sheets/Collections-{current_date}.xlsx', engine='xlsxwriter')
	df.to_excel(writer, sheet_name='Collections', startrow=1, header=False, index=False)
	...
	new_doc.report_name = f'Collections-{current_date}'
	new_doc.insert()
	url = [{'file_url' : ''}]
	
	file_data = open(f'/home/frappe/frappe-bench/apps/erpnext/erpnext/selling/report/automated_reports_for_logistics/excel_sheets/Collections-{current_date}.xlsx', 'rb').read()
	file_attach = save_file(f'Collections-{current_date}.xlsx', file_data, 'Logistics Report Attachments', new_doc.name)
	...
	subject = f'Collections Report - {current_date}'
	message = '''Hello Team,<br>
	<br>
	PFA, for the updated Collections report as of 7:00 AM today.'''
	frappe.sendmail(
