...
def test():
	...
	report_filters = {'company': 'VIBGYOR NET CONNECTIONS PVT LTD', 'from_date': from_date, 'to_date': to_date, 'item_group': 'Products'}
	columns, report_data, var1, var2, var3, total_rows = execute(filters=report_filters)
	existing_doc = frappe.get_list('Item-Wise Sales Register Custom Report', filters = {'from_date': str(from_date),
																					'to_date': str(to_date)
	})
	if existing_doc:
		so_doc = frappe.get_doc('Item-Wise Sales Register Custom Report', f'{str(from_date)}-{str(to_date)}')
	else:
		so_doc = frappe.new_doc('Item-Wise Sales Register Custom Report')
	...
	for item in report_data:
		...
		if sales_order:
			if sales_order != prev:
				sales_order = item['sales_order']
				so1_doc = frappe.get_doc('Sales Order', sales_order)
				customer_po = so1_doc.po_no
				
			else:
				customer_po = None
			prev = item['sales_order']
			report_values.append({'item_code': item['item_code'],
								...
								'receivable_account': item['debit_to'],
								'mode_of_payment': item['mode_of_payment'],
								'territory': item['territory'],
								'project': item['project'],
								'company': item['company'],
								'sales_order': item['sales_order'],
								'customers_purchase_order': customer_po,
								...
								'rate': item['rate'],
								'amount': item['amount'],
								'output_tax_cgst__90_rate': item['output_tax_cgst_@_9.0_rate'],
								'output_tax_cgst__90_amount': item['output_tax_cgst_@_9.0_amount'],
								'output_tax_igst__180_rate': item['output_tax_igst_@_18.0_rate'],
								'output_tax_igst__180_amount': item['output_tax_igst_@_18.0_amount'],
								'output_tax_sgst__90_rate': item['output_tax_sgst_@_9.0_rate'],
								'output_tax_sgst__90_amount': item['output_tax_sgst_@_9.0_amount'],
								'total_tax': item['total_tax'],
								'total_other_charges': item['total_other_charges'],
								'total': item['total']
			})
	...
	writer = pd.ExcelWriter(f'/home/frappe/frappe-bench/apps/erpnext/erpnext/selling/report/automated_reports_for_logistics/excel_sheets/Itemwise and As per Sales-{current_date}.xlsx', engine='xlsxwriter')
	df.to_excel(writer, sheet_name='Itemwise Sales Register', startrow=1, header=False, index=False)
	workbook = writer.book
	worksheet = writer.sheets['Itemwise Sales Register']
	...
	parent = f'{from_date}-{to_date}'

	#Generate As per sales report
	summary = frappe.db.sql(f'''select sales_order, customer_name, customers_purchase_order, sum(amount)
							from `tabItem-wise Sales Register Child Table` where parent = '{parent}'
							AND (sales_order IS NOT NULL OR customers_purchase_order IS NOT NULL OR customer_name IS NOT NULL)
							group by sales_order, customer_name, customers_purchase_order;
							
	''')

	existing_doc = frappe.get_list('As per Sales Monthly Summary', filters = {'from_date': str(from_date),
																					'to_date': str(to_date)
	})
	if existing_doc:
		so_doc = frappe.get_doc('As per Sales Monthly Summary', f'{str(from_date)}-{str(to_date)}')
	else:
		so_doc = frappe.new_doc('As per Sales Monthly Summary')
		so_doc.from_date = str(from_date)
		so_doc.to_date = str(to_date)

	report_values1 = []    
	for item in summary:
		report_values1.append({'sales_order': item[0],
							'customer_name': item[1],
							'purchase_order': item[2],
							'total_sale': item[3]
		})
	...
	so_doc.set('report', {})
	so_doc.extend('report', report_values1)
	so_doc.save()

	df1.to_excel(writer, sheet_name='As Per Sales', startrow=1, header=False, index=False)
	second_worksheet = writer.sheets['As Per Sales']
	...
	new_doc.report_name = f'Itemwise and As per Sales-{current_date}'
	new_doc.insert()
	url = [{'file_url' : ''}]
	
	file_data = open(f'/home/frappe/frappe-bench/apps/erpnext/erpnext/selling/report/automated_reports_for_logistics/excel_sheets/Itemwise and As per Sales-{current_date}.xlsx', 'rb').read()
	file_attach = save_file(f'Itemwise and As per Sales-{current_date}.xlsx', file_data, 'Logistics Report Attachments', new_doc.name)
	...
	subject = f'Itemwise Sales Register and As per Sales Monthly report - {current_date}'
	message = '''Hello Team,<br>
	<br>
	PFA, for the updated Itemwise Sales Register and As per Sales Monthly report as of 7:00 AM today.'''
	frappe.sendmail(
