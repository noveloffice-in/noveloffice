...
def two_days_after_updating_result():
    today = frappe.utils.getdate()
    two_days_before = frappe.utils.add_days(today, -2)
    candidates = frappe.get_all('Candidates', filters={'result1': 'Rejected',"last_rejected_date":two_days_before}, fields=['name','primary_email','last_rejected_date'])
    # candidates = frappe.get_all('Candidates', filters={'result1': ['in', ['Future Hire', 'Rejected']]}, fields=['name','primary_email','last_rejected_date'])
    for candidate in candidates:
        try:
            last_rejected_date = candidate.get('last_rejected_date')
            ...
            success_doc.script_name = 'Rejection mail to candidates 2 days after updating result'
            success_doc.save()  
            ...
            error_doc.script_name = 'Rejection mail to candidates 2 days after updating result'
            error_doc.error = str(e)
            error_doc.save()

def two_days_after_updating_result_enqueue():
    frappe.enqueue(two_days_after_updating_result, queue = 'long')